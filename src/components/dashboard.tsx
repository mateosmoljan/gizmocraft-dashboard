    ...
    <div className="space-y-6">
        <header className="flex flex-col gap-4 rounded-3xl border border-emerald-300/20 bg-white/8 p-6 shadow-2xl shadow-black/30 backdrop-blur md:flex-row md:items-end md:justify-between">
          ...
          <div className="rounded-2xl border border-lime-300/30 bg-lime-300/10 px-5 py-4 text-right">
            ...
            <p className="text-xl font-bold">{worldStats.name}</p>
            <p className="text-sm text-slate-300">{worldStats.difficulty} · {worldStats.trackedPlayers} players tracked</p>
          </div>
        </header>

        <div className="grid gap-4 md:grid-cols-4">
          {[['Online', worldStats.playersOnline + '/' + worldStats.maxPlayers], ['Top score', format(players[0].score)], ['Last sync', worldStats.lastSync], ['Mode', 'Public boards']].map(([k,v]) => (
