...
export function ProfileSettings({ profile }: { profile: Profile }) {
  ...
  async function save() {
    ...
    setStatus(res.ok ? "Saved" : "Could not save profile");
  }
    ...
    <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/8 p-5 backdrop-blur">
        ...
        <div className="mt-6 grid gap-4">
          ...
          <label className="grid gap-2 text-sm text-slate-300">Picture URL<input className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white" value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} /></label>
